  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6, marginBottom: 10 }}>
        {[
          ['ENTRY',  alert.entry  ? `$${fmt(alert.entry)}`  : `$${fmt(alert.price)}`, C.text],
          ['STOP',   alert.stop   ? `$${fmt(alert.stop)}`   : '—',                    C.short],
          ['TARGET', alert.target ? `$${fmt(alert.target)}` : '—',                    C.long],
          ['R:R',    alert.rr ? `${alert.rr}R` : '—',                                 C.gold],
        ].map(([l, v, col]) => (
...
export function DataBadge({ isLive, dataMode }) {
  // LIVE    = Massive.com real-time → teal
  // DELAYED = Yahoo Finance (stocks + futures, ~15 min delay) → purple
  const c = isLive ? C.teal : '#a78bfa'

  const label = isLive ? '● LIVE' : '◑ DELAYED'

  const tip = isLive
    ? 'Real-time via Massive.com WebSocket'
    : 'Stocks & Futures: Yahoo Finance (~15 min delayed) · Crypto: Binance live · Add Massive.com key for full real-time'
...
export function Toast({ msg, type }) {
  ...
  // Split off the price portion (everything after " · E $") for colour emphasis
  const [mainPart, pricePart] = msg.split(' · E $')
  const hasPrices = pricePart !== undefined
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        {hasPrices ? mainPart : msg}
      </div>
      {hasPrices && (
        <div style={{ fontFamily: 'monospace', fontSize: 11, marginTop: 3, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <span>ENTRY <span style={{ color: C.text, fontWeight: 700 }}>${pricePart.split(' → T $')[0]}</span></span>
          {pricePart.includes(' → T $') && (
            <span>TARGET <span style={{ color: C.long, fontWeight: 700 }}>${pricePart.split(' → T $')[1]}</span></span>
          )}
        </div>
      )}
    </div>
