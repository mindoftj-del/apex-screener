...
import { findSwingLevels, fmt, getKillZoneStatus, isLunchBlackout } from './technicals.js'

// Tick buffer — same logic as technicals.js (duplicated to avoid circular import)
function buf(price) {
  if (price > 10000) return price * 0.0003
  if (price > 1000)  return price * 0.0005
...
export function detectICTSignals({ sym, market, candles, price, vwap, rsi, volRatio, structure, riskPct, accountSize, peerCandles }) {
  ...
  // ── Structural levels (used for stop and target calculation) ─────────────────
  ...
  const addSig = (sig) => {
    // ── BACKTEST GATES ─────────────────────────────────────────────────────────
    if (isLunchBlackout()) return
    if (sig.grade === 'A+' && !killZone.active) return

    // Minimum stop distance — 8 NQ pts (prevents near-zero loss anomalies in OB/FVG)
    if (sig.stop && sig.entry && Math.abs(sig.entry - sig.stop) < 8) return
    ...
    // entry defaults to current price if not set by the signal
  ...
  if (sweep) {
    ...
    // Backtest: sweep only fires during kill zones (win rate collapses off-hours)
    if ((sweepFVG || (sweep && volRatio > 1.5)) && killZone.active) {
      ...
      addSig({
        ...
        // Entry: above sweep candle high (LONG) or below sweep candle low (SHORT)
        // Stop: beyond the sweep wick — if it holds below there, the sweep failed
        // Target: opposite swing level (the draw on liquidity that caused the sweep)
        entry:  isBull ? lastCandle.high + buf(price) : lastCandle.low  - buf(price),
        stop:   isBull ? sweep.sweepLow  - buf(price) : sweep.sweepHigh + buf(price),
        target: isBull ? (farHigh > price ? farHigh : swingHigh) : (farLow < price ? farLow : swingLow),
        stopNote:   isBull ? `Below sweep wick $\${fmt(sweep.sweepLow)} — stop hunt reversal invalidated` : `Above sweep wick $\${fmt(sweep.sweepHigh)}`,
        targetNote: `Prior swing $\${fmt(isBull ? farHigh : farLow)} — the draw on liquidity`,
        rule: sweep.rule,
        source: 'ICT',
  ...
  if (activeOB && volRatio > 1.2 && structure.mtfScore >= 1) {
    ...
    addSig({
      ...
      // Entry: limit order at OB midpoint (mean threshold)
      // Stop: beyond OB boundary — if price closes outside the OB it's invalidated
      // Target: opposite swing or next FVG
      entry:  activeOB.mid,
      stop:   isBull ? activeOB.obLow  - buf(price) : activeOB.obHigh + buf(price),
      target: isBull ? (swingHigh > price ? swingHigh : farHigh) : (swingLow < price ? swingLow : farLow),
      stopNote:   isBull ? `Below OB low $\${fmt(activeOB.obLow)}` : `Above OB high $\${fmt(activeOB.obHigh)}`,
      targetNote: `Prior swing $\${fmt(isBull ? swingHigh : swingLow)}. 50% off at midpoint.`,
      rule: activeOB.rule,
      source: 'ICT',
      killZone: killZone.name,
    })
  ...
  if (activeFVG && volRatio > 1.1 && rsi > 30 && rsi < 70) {
    ...
    addSig({
      ...
      // Stop: beyond the far edge of the FVG — gap is fully filled
      // Target: swing on the other side (FVGs pull price toward prior swings)
      entry:  activeFVG.mid,
      stop:   isBull ? activeFVG.gapLow  - buf(price) : activeFVG.gapHigh + buf(price),
      target: isBull ? (swingHigh > activeFVG.gapHigh ? swingHigh : farHigh) : (swingLow < activeFVG.gapLow ? swingLow : farLow),
      stopNote:   isBull ? `Below FVG low $\${fmt(activeFVG.gapLow)} — gap fully filled` : `Above FVG high $\${fmt(activeFVG.gapHigh)}`,
      targetNote: `Prior swing $\${fmt(isBull ? swingHigh : swingLow)} beyond the FVG`,
      rule: activeFVG.rule,
      source: 'ICT',
      killZone: killZone.name,
    })
  ...
  if (activeIFVG && Math.abs((price - activeIFVG.mid) / activeIFVG.mid) < 0.002) {
    ...
    addSig({
      ...
      // Entry: at IFVG midpoint — polarity flipped so old gap edge = new S/R
      // Stop: beyond the IFVG entirely — flip has failed
      // Target: next swing (IFVGs have high follow-through)
      entry:  activeIFVG.mid,
      stop:   isBull ? activeIFVG.gapLow  - buf(price) : activeIFVG.gapHigh + buf(price),
      target: isBull ? (farHigh > price ? farHigh : swingHigh) : (farLow < price ? farLow : swingLow),
      stopNote:   isBull ? `Below IFVG low $\${fmt(activeIFVG.gapLow)}` : `Above IFVG high $\${fmt(activeIFVG.gapHigh)}`,
      targetNote: `Next swing $\${fmt(isBull ? farHigh : farLow)} — IFVG signals full reversal`,
  ...
  if (breaker && Math.abs((price - breaker.level) / breaker.level) < 0.003) {
    ...
    addSig({
      ...
      // Entry: at breaker level (where failed OB now acts as S/R)
      // Stop: beyond the breaker candle extreme — structure shift negated
      // Target: prior swing high/low (the draw)
      entry:  breaker.level,
      stop:   isBull ? (breaker.low  || breaker.level * 0.998) - buf(price) : (breaker.high || breaker.level * 1.002) + buf(price),
      target: isBull ? (swingHigh > price ? swingHigh : farHigh) : (swingLow < price ? swingLow : farLow),
      stopNote:   `Beyond breaker extreme $\${fmt(isBull ? breaker.low : breaker.high)}`,
      targetNote: `Prior swing $\${fmt(isBull ? swingHigh : swingLow)}`,
      rule: breaker.rule,
      source: 'ICT',
      killZone: killZone.name,
    })
  ...
  if (vpLevels && Math.abs(vpLevels.pocDev) > 0.8 && volRatio > 1.2) {
    ...
    addSig({
      ...
      // Entry: market entry — price is extended from POC now
      // Stop: beyond current candle extreme (if it keeps going, reversion failed)
      // Target: POC itself (mean reversion = exactly to the highest volume price)
      entry:  price,
      stop:   isBull ? lastCandle.low - buf(price) : lastCandle.high + buf(price),
      target: vpLevels.poc,
      stopNote:   `Below/above current candle $\${fmt(isBull ? lastCandle.low : lastCandle.high)} — reversion failed`,
      targetNote: `POC $\${fmt(vpLevels.poc)} — highest volume price, always the magnet`,
  ...
  if (vpLevels?.atVAH && rsi > 60 && volRatio > 1.1) {
    addSig({
      ...
      // Entry: at VAH (value area high) with price rejecting it
      // Stop: above VAH — market accepted value higher, trade is wrong
      // Target: POC first, then VAL
      entry:  price,
      stop:   vpLevels.vah + buf(vpLevels.vah),
      target: vpLevels.poc,
      stopNote:   `Above VAH $\${fmt(vpLevels.vah)} — value accepted higher`,
      targetNote: `POC $\${fmt(vpLevels.poc)} then VAL $\${fmt(vpLevels.val)}`,
  ...
  if (vpLevels?.atVAL && rsi < 40 && volRatio > 1.1) {
    addSig({
      ...
      entry:  price,
      stop:   vpLevels.val - buf(vpLevels.val),
      target: vpLevels.poc,
      stopNote:   `Below VAL $\${fmt(vpLevels.val)} — value accepted lower`,
      targetNote: `POC $\${fmt(vpLevels.poc)} then VAH $\${fmt(vpLevels.vah)}`,
  ...
  // Backtest: IB Breakout only valid in first 45 minutes of NY session
  const nowET_min = (() => { const n=new Date(); return ((n.getUTCHours()-5+24)%24)*60+n.getUTCMinutes() })()
  const inIBWindow = nowET_min >= 570 && nowET_min <= 615  // 9:30–10:15 ET
  if (mpLevels && isNQorES && inIBWindow) {
    if (mpLevels.atIBHigh && volRatio > 1.5 && !mpLevels.extendedUp) {
      addSig({
        ...
        // Entry: close above IB High (breakout confirmed)
        // Stop: back below IB High — failed breakout, no follow-through
        // Target: IB High + IB Range (measured move — classic Market Profile projection)
        entry:  lastCandle.close,
        stop:   mpLevels.ibHigh - buf(mpLevels.ibHigh),
        target: mpLevels.ibHigh + mpLevels.ibRange,
        stopNote:   `Below IB High $\${fmt(mpLevels.ibHigh)} — failed breakout`,
        targetNote: `IB High + Range = $\${fmt(mpLevels.ibHigh + mpLevels.ibRange)} (measured move)`,
    ...
    if (mpLevels.atIBLow && volRatio > 1.5 && !mpLevels.extendedDown) {
      addSig({
        ...
        entryLabel: 'T1 · MARKET PROFILE',
        entry:  lastCandle.close,
        stop:   mpLevels.ibLow + buf(mpLevels.ibLow),
        target: mpLevels.ibLow - mpLevels.ibRange,
        stopNote:   `Above IB Low $\${fmt(mpLevels.ibLow)} — failed breakdown`,
        targetNote: `IB Low − Range = $\${fmt(mpLevels.ibLow - mpLevels.ibRange)} (measured move)`,
  ...
  if (smt && killZone.active) {
    ...
    addSig({
      ...
      // Entry: market entry — SMT is a real-time divergence, enter now
      // Stop: beyond the sweep wick of the diverging instrument (use current candle)
      // Target: opposite liquidity pool (prior swing)
      entry:  price,
      stop:   isBull ? lastCandle.low  - buf(price) : lastCandle.high + buf(price),
      target: isBull ? (farHigh > price ? farHigh : swingHigh) : (farLow < price ? farLow : swingLow),
      stopNote:   isBull ? `Below sweep candle low $\${fmt(lastCandle.low)}` : `Above sweep candle high $\${fmt(lastCandle.high)}`,
      targetNote: `Opposite liquidity pool $\${fmt(isBull ? farHigh : farLow)}`,
      rule: smt.rule,
      source: 'ICT',
      killZone: killZone.name,
    })
  ...
  if (vpLevels?.lvns?.length > 0) {
    ...
    if (nearLVN && volRatio > 1.3 && structure.trend !== 'RANGING') {
      ...
      addSig({
        ...
        entry:  price,
        stop:   isBull ? nearLVN - buf(nearLVN) : nearLVN + buf(nearLVN),
        target: isBull ? (vpLevels.hvns.find(h => h > price) || swingHigh) : ([...vpLevels.hvns].reverse().find(h => h < price) || swingLow),
        stopNote:   `Back through LVN $\${fmt(nearLVN)} — momentum failed`,
        targetNote: `Next HVN $\${fmt(isBull ? (vpLevels.hvns.find(h => h > price) || swingHigh) : ([...vpLevels.hvns].reverse().find(h => h < price) || swingLow))}`,
