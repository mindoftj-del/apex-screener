...
import { detectSignals, fmt, getKillZoneStatus, isLunchBlackout } from './engine/technicals.js'
import { detectICTSignals } from './engine/ict.js'
...
function KZBadge({ kz }) {
  if (!kz) return null
  const isBlackout = kz.quality === 'BLACKOUT'
  return (
    <span title={isBlackout ? 'Backtest shows near-zero edge during lunch — signals paused' : `${kz.quality} probability window`}
      style={{ fontSize: 10, padding: '3px 8px', borderRadius: 4, fontWeight: 700, cursor: 'default',
        background: isBlackout ? '#ff3b5c18' : kz.active ? `${kz.color}18` : '#ffffff06',
        border: `1px solid ${isBlackout ? '#ff3b5c44' : kz.active ? kz.color + '44' : C.border}`,
        color: isBlackout ? '#ff3b5c' : kz.active ? kz.color : C.textFaint }}>
      {isBlackout ? '⛔ LUNCH — NO SIGNALS' : kz.active ? '🎯 ' + kz.name : '○ ' + kz.name}
    </span>
  )
}
...
export default function App() {
  ...
  const [killZone, setKillZone] = useState(() => getKillZoneStatus())

  // Kill zone ticker
  useEffect(() => {
    const id = setInterval(() => setKillZone(getKillZoneStatus()), 30000)
    ...
    ALL_SYMBOLS.forEach(({ sym, market }) => {
      ...
      ;[...cs.signals, ...ict.ictSignals].forEach(sig => {
        ...
        if (sig.grade === 'A+' || sig.grade === 'A') {
          const entryStr  = sig.entry  ? ` · E $${sig.entry.toLocaleString('en-US', { maximumFractionDigits: 2 })}` : ''
          const targetStr = sig.target ? ` → T $${sig.target.toLocaleString('en-US', { maximumFractionDigits: 2 })}` : ''
          addToast(
            `${sig.dir === 'LONG' ? '▲' : sig.dir === 'SHORT' ? '▼' : '◆'} ${sym} ${sig.grade} — ${sig.reason}${entryStr}${targetStr}`,
            sig.grade === 'A+' ? 'ap' : 'signal'
          )
        }
      })
    })
